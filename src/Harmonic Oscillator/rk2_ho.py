...
#|      for the harmonic oscillator problem using the Runge-Kutta 2nd Order method.      |
...
# Applying 2nd Order Runge-Kutta Algorithm for Dynamic Evolution:
for i in range(steps):
    ...
    # Calculating the next steps (complete solution) based only on k2:
    x[i+1]  = x[i]  + k2_x
    y[i+1]  = y[i]  + k2_y
    vx[i+1] = vx[i] + k2_vx
    vy[i+1] = vy[i] + k2_vy
