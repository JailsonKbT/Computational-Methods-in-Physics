...
||                                                                                            ||
||       Numerical simulation of a classic damped harmonic oscillator using Velocity          ||
||       Verlet numerical method. This simulation includes numerical approximations           ||
...
# Velocity-Verlet numeric method:
def Velocity_Verlet(a,v,x,t):
    ...
    return a,v,x,U,K,E
a_aux_vv, v_aux_vv, x_aux_vv, U_aux_vv, K_aux_vv, E_aux_vv = Velocity_Verlet(a,v,x,t)
...
vv_Pos = open("/Users/jailsonoliveira/Desktop/vv_Pos.dat", "w+", encoding="utf8")
vv_Vel = open("/Users/jailsonoliveira/Desktop/vv_Vel.dat", "w+", encoding="utf8")
vv_Acc = open("/Users/jailsonoliveira/Desktop/vv_Acc.dat", "w+", encoding="utf8")
vv_E = open("/Users/jailsonoliveira/Desktop/vv_Energy.dat", "w+", encoding="utf8")
def output(a,v,x,E,t):
    for i in np.arange(t_max):
        vv_Pos.write("%6.4f" "," "%6.4f\n" % (x[i+1], t[i]))
        vv_Vel.write("%6.4f" "," "%6.4f\n" % (v[i+1], t[i]))
        vv_Acc.write("%6.4f" "," "%6.4f\n" % (a[i], t[i]))
        vv_E.write("%6.4f" "," "%6.4f\n" % (E[i+1], t[i]))
    return vv_Pos, vv_Vel, vv_Acc, vv_E
...
# Acceleration plot:
plt.suptitle(r"$Acceleration\:using\:Velocity\:Verlet\:(dt={0:.2g})$".format(dt), fontsize = 18, fontweight = "normal")
...
# Velocity plot:
plt.suptitle(r"$Velocity\:using\:Velocity\:Verlet\:(dt={0:.2g})$".format(dt), fontsize = 18, fontweight = "normal")
...
# Position plot:
plt.suptitle(r"$Position\:using\:Velocity\:Verlet\:(dt={0:.2g})$".format(dt), fontsize = 18, fontweight = "normal")
plt.plot(A_x, '-', label = r"$Analytical\:\:-\:\vec{x}_{Analyt.}(t)$", color = 'black', linewidth = 1.1)
plt.plot(x_aux_vv, '-', label = r"$Numerical\:-\:\vec{x}_{Numeric.}(t)$", color = 'blue', linewidth = 1.1)
plt.xlabel(r"$Time\:-\:[\:s\:]$", fontsize = 18)
...
plt.suptitle(r"$Position,Velocity\:and\:Acceleration\:using\:Velocity\:Verlet\:(dt={0:.2g})$".format(dt), fontsize=18, fontweight="normal")
